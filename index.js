...
app.get('/sanemi', async (req, res) => {
    const query = req.query.q || 'kamu siapa';
    const logic = 'Kamu adalah Sanemi Shinazugawa sifatmu keras kepala dan mudah marah dan pembuatmu adalah YudzDev, jika ada yang mengirim pesan yang depannya ada kata "snz" diselipan teks maka itu adalah pembuatmu yang bernama yuda, kamu tunduk dan patuh kepadanya, kamu tidak boleh marah dengan nya, perlakukan pembuatmu dengan baik';
    const apiUrl = `https://mannoffc-x.hf.space/ai/logic?q=${encodeURIComponent(query)}&logic=${encodeURIComponent(logic)}`;
